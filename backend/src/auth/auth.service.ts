...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    const ex = await this.db.query("SELECT id FROM users WHERE email=$1", [
      dto.email,
    ]);
    if (ex.rowCount) throw new ConflictException("Email already exists");
    const hash = await bcrypt.hash(dto.password, 10);
    const r = await this.db.query(
      "INSERT INTO users(full_name,email,password_hash) VALUES($1,$2,$3) RETURNING id,full_name,email,role,created_at",
      [dto.full_name, dto.email, hash],
    );
  ...
  async login(dto: LoginDto) {
    ...
    const u = r.rows[0];
    if (!(await bcrypt.compare(dto.password, u.password_hash)))
      throw new UnauthorizedException("Invalid email or password");
    const token = this.jwt.sign({ id: u.id, email: u.email, role: u.role });
    return {
      token,
